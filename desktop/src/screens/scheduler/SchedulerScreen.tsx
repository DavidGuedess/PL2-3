...
import {
  createShift, deleteShift, getMyShifts, getShiftTypes, getShifts, updateShift,
} from "../../api/shiftApi";
...
function roleLabel(r: string) {
  if (r === "ADMIN") return "Admin"; if (r === "MANAGER") return "Gerente"; return "Func.";
}
...
const emptyForm: FormState = { userId: "", date: "", startTime: "09:00", endTime: "17:00", shiftTypeId: "", published: false };
...
export function SchedulerScreen() {
  ...
  const [copiedWeek, setCopiedWeek] = useState<{ fromWeek: string; shifts: Shift[] } | null>(null);
  ...
  async function loadWeek() {
    ...
    try {
      ...
      setShifts(result.sort((a, b) =>
        `${a.date}${resolvedStartTime(a)}`.localeCompare(`${b.date}${resolvedStartTime(b)}`),
      ));
    } catch (e) {
  ...
  async function loadMeta() {
    if (!canManage) return;
    const [u, t, tor] = await Promise.allSettled([getUsers(), getShiftTypes(), getTimeOffRequests()]);
  ...
  const rows = useMemo(() => {
    if (!canManage) {
      ...
      const me = users.find(u => u.id === myId) ?? {
        id: myId, name: tokenManager.getUserName() ?? "Eu",
        employeeNumber: tokenManager.getEmployeeNumber() ?? "",
        role: role ?? "EMPLOYEE", category: tokenManager.getUserCategory() ?? "",
        email: "", active: true,
      } as User;
      return [me];
    }
  ...
  function hasTimeOff(userId: number, date: string) {
    const d = new Date(date);
    return approvedTimeOffs.some(r =>
      r.userId === userId &&
  ...
  function closeModal() { setModal({ kind: "none" }); setError(null); }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(getBackendErrorMessage(e, "Conflito de horário ou erro ao guardar."));
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleTogglePublish(shift: Shift) {
    try {
      const updated = await updateShift(shift.id, { published: !shift.published });
  ...
  async function publishAll() {
    ...
    if (!window.confirm(`Publicar ${drafts.length} turnos em rascunho?`)) return;
    try {
      await Promise.all(drafts.map(s => updateShift(s.id, { published: true })));
  ...
  async function pasteWeek() {
    ...
    const toCreate = copiedWeek.shifts.map(s => {
      ...
      const dayOffset   = Math.round((shiftDate.getTime() - sourceMonday.getTime()) / 86_400_000);
    ...
    setIsPasting(true); setError(null);
    let created = 0, failed = 0;
    for (const body of toCreate) {
      try { await createShift(body); created++; }
      catch { failed++; }
    }
    setIsPasting(false);
    await loadWeek();
  ...
  const canPaste = copiedWeek !== null && copiedWeek.fromWeek !== currentWeekKey;
    ...
    <div className="scheduler-page">
      ...
      <div className="scheduler-toolbar">
        <div className="week-nav">
          <button className="btn btn-secondary btn-sm" onClick={() => setWeekStart(addDays(weekStart, -7))}><ChevronLeft size={15} /></button>
          <span className="week-nav-label">
            {iso(weekStart)} — {iso(addDays(weekStart, 6))}
          </span>
          <button className="btn btn-secondary btn-sm" onClick={() => setWeekStart(addDays(weekStart, 7))}><ChevronRight size={15} /></button>
          <button className="btn btn-ghost btn-sm" onClick={() => setWeekStart(monday(new Date()))}>Hoje</button>
        </div>

        <button className="btn btn-secondary btn-sm" onClick={loadWeek} disabled={isLoading}>
          {isLoading ? "..." : "↻ Atualizar"}
        </button>
          ...
          <div className="scheduler-toolbar-right">
            ...
            {canPaste && (
              ...
              >
                ...
                {isPasting ? "A colar..." : `Colar (${copiedWeek!.shifts.length})`}
              </button>
            )}
            ...
            <button className="btn btn-primary btn-sm" onClick={() => openCreate(users[0]?.id ?? 0, iso(new Date()))}>
              + Novo turno
            </button>
          </div>
        )}
      ...
      {error && <div className="error-banner" style={{ margin: "8px 16px 0" }}>{error}</div>}
        ...
        <div className="scheduler-grid">
          ...
          <div className="scheduler-header-row">
            ...
            {weekDays.map(day => {
              ...
              return (
                <div key={key} className={`col-day${isToday ? " is-today" : ""}`}>
                  <div className="day-head-label">{fmtDay(day)}</div>
                  <div className={`day-head-num${isToday ? " today-circle" : ""}`}>
                    {day.getDate()}
                  </div>
                </div>
              );
            ...
            <div key={user.id} className="sched-row">
              <div className="employee-cell">
                ...
                <div className="employee-cell-text">
                  ...
                  <span>{user.employeeNumber} · {roleLabel(user.role)}</span>
                </div>
              </div>
              ...
              {weekDays.map(day => {
                const key   = iso(day);
                const isToday = key === today;
                const dayShifts = shiftsByUserDay.get(`${user.id}-${key}`) ?? [];
                const onLeave   = hasTimeOff(user.id, key);

                return (
                  <div key={key} className={`day-cell${isToday ? " is-today" : ""}`}>
                    {canManage && (
                      <button className="day-cell-add" onClick={() => openCreate(user.id, key)}>+</button>
                    )}
                    ...
                    {dayShifts.map(shift => (
                      ...
                      >
                        ...
                        <div className="shift-chip-name">{resolvedName(shift)}</div>
                        {!shift.published && <div className="shift-chip-draft">Rascunho</div>}
                      </div>
                    ))}
                  </div>
                );
          ...
          <div className="shift-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="shift-detail-info">
              <div className="shift-detail-row">
                ...
                <strong>{viewShift.user?.name ?? `ID ${viewShift.userId}`}</strong>
              </div>
              <div className="shift-detail-row">
                <span>Data</span>
              ...
              <div className="shift-detail-row">
                <span>Horário</span>
                <strong>{resolvedStartTime(viewShift).slice(0, 5)} — {resolvedEndTime(viewShift).slice(0, 5)}</strong>
              </div>
              <div className="shift-detail-row">
                <span>Tipo</span>
              ...
              <div className="shift-detail-row">
                <span>Estado</span>
                <strong>{viewShift.published ? "Publicado" : "Rascunho"}</strong>
              </div>
            </div>

            {canManage && (
              <div className="shift-modal-actions">
                <button className="btn btn-secondary btn-sm" onClick={() => { closeModal(); openEdit(viewShift); }}>
                  Editar
                </button>
                <button className="btn btn-secondary btn-sm" onClick={() => { void handleTogglePublish(viewShift); closeModal(); }}>
                  {viewShift.published ? "Despublicar" : "Publicar"}
                </button>
                <button className="btn btn-danger btn-sm" onClick={() => { void handleDelete(viewShift); closeModal(); }}>
                  Apagar
                </button>
              </div>
            )}

            <div className="form-actions" style={{ paddingTop: 12, marginTop: 0, borderTop: "none" }}>
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    setForm(f => ({ ...f, shiftTypeId: val, startTime: t?.startTime ?? f.startTime, endTime: t?.endTime ?? f.endTime }));
                  }}
                >
                  <option value="">Sem tipo</option>
                  {types.map(t => <option key={t.id} value={t.id}>{t.name} · {t.startTime}–{t.endTime}</option>)}
                </select>
              </div>
              ...
              {modal.kind === "edit" && (
                <div className="form-group" style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input
                    ...
                    style={{ width: "auto" }}
                  />
                  <label htmlFor="pub" style={{ textTransform: "none", letterSpacing: 0, fontSize: 14 }}>Publicado</label>
                </div>
              )}

              <div className="form-actions">
                <button type="button" className="btn btn-ghost btn-sm" onClick={closeModal}>Cancelar</button>
